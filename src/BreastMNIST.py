...
data_flag = 'breastmnist'  # for example, use 'pathmnist' for patholog images
...
train_dataset = BreastMNIST(split='train', download=True, transform=transformations)
test_dataset = BreastMNIST(split='test', download=True, transform=transformations)
...
def consistency_feature_importance(
    ...
    plt.tight_layout()
    plt.savefig(save_dir / "BreastMNIST_consistency_features.pdf")
    plt.close()
